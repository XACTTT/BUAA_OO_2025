...
class Judger:
    ...
    def init(self, data_filename, result_filename, judge_filename):
        ...
        self.elevator_list = [
            {"floor": 0, "passenger": 0, "status": _CLOSE, "open_time": None}
    ...
    def parse_data(self, data_lines):
        for line_num, line in enumerate(data_lines, 1):
            line = line.strip()
            pattern = r'\[\s*([0-9]+\.[0-9]+)\s*\]([0-9]+)-PRI-([0-9]+)-FROM-(B4|B3|B2|B1|F1|F2|F3|F4|F5|F6|F7)-TO-(B4|B3|B2|B1|F1|F2|F3|F4|F5|F6|F7)-BY-([1-6])'
            match = re.fullmatch(pattern, line)
            if not match:
                self.report_error("FormatError", f"Input line {line_num} format invalid: {line}")
                return False

            _, pid, pri, from_floor, to_floor, eid = match.groups()
            eid = int(eid)
            # 检查楼层有效性
            if from_floor not in self.floor_map or to_floor not in self.floor_map:
                self.report_error("FormatError", f"Invalid floor in line {line_num}: {line}")
                return False
...
#        pattern = r'\[\s*([0-9]+\.[0-9]+)\]\s*CLOSE-([B4B3B2B1F1F2F3F4F5F6F7]+)-([1-6])'
    ...
    def judge_out(self, line, line_num):
        pattern = r'\[\s*([0-9]+\.[0-9]+)\s*\]\s*OUT-(\d+)-(B4|B3|B2|B1|F1|F2|F3|F4|F5|F6|F7)-([1-6])'
        match = re.fullmatch(pattern, line)
        if not match:
        ...
        if not passenger["entered"]:
            ...
            return False
        # 检查目标楼层
        if passenger["to"] != elevator["floor"]:
    ...
    def check_all_passengers(self):
        for pid, info in self.passenger_dict.items():
            if not (info["entered"] and info["exited"]):
